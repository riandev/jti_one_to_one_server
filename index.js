...
const app = express();
app.use(express.static("../jti_one_to_one_client/build"));
app.use(cors());
...
// const MongoClient = require("mongodb").MongoClient;
const uri = "mongodb://127.0.0.1:27017/jti_one_2_one";
// const uri =
//   "mongodb+srv://aktcl:01939773554op5t@cluster0.9akoo.mongodb.net/jti_one_2_one?retryWrites=true&w=majority";
const client = new MongoClient(uri, {
  useNewUrlParser: true,
  useUnifiedTopology: true,
});
client.connect((err) => {
  const userCollection = client.db("jti_one_2_one").collection("users");
  const adminCollection = client.db("jti_one_2_one").collection("admins");
  const leadsCollection = client.db("jti_one_2_one").collection("leads");
      ...
      .updateOne(
        { _id: id },
        {
          $set: {
            ...
            answer13: update.answer13,
            answer14: update.answer14,
            answer15: update.answer15,
            answer16: update.answer16,
            answer17: update.answer17,
            answer18: update.answer18,
      ...
      .toArray((err, results) => {
        ...
        for (user in users) {
          output.push({
            ...
            twoStick_sales: users[user]
              .filter((x) => x.sales_status === "2_sticks_trial")
              .slice(0, 1)
              .map((d) => {
                let datas = {};
                (datas.id = d._id),
            ...
            vao_sales: users[user]
              .filter((x) => x.sales_status === "40_sticks_with_VAO")
              .slice(0, 5)
              .map((d) => {
                let datas = {};
                (datas.id = d._id),
            ...
            oneStick_sales: users[user]
              ...
              .slice(
                0,
                users[user].filter(
                  (x) => x.sales_status === "40_sticks_with_VAO"
                ).length < 5
                  ? 5 -
                      users[user].filter(
                        (x) => x.sales_status === "40_sticks_with_VAO"
  ...
  app.get("*", (req, res) => {
    res.sendFile(
      path.join(__dirname, "../jti_one_to_one_client/build", "index.html")
    );
  });
});
